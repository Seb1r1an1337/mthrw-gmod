...
namespace Math {
    inline Angle CalculateAngle(const Vector& src, const Vector& dst) {
        Angle angles;
        Vector delta = src - dst;
        float hyp = std::sqrtf(delta.x * delta.x + delta.y * delta.y);

        angles.p = std::atan2f(delta.z, hyp) * (180.0f / M_PI_F);
        angles.y = std::atan2f(delta.y, delta.x) * (180.0f / M_PI_F) + 180.0f;
        angles.r = 0.0f;

        return angles;
    }

    inline float GetFov(const Angle& viewAngle, const Angle& aimAngle) {
        Angle delta = aimAngle - viewAngle;
        delta.FixAngles();
        return std::sqrtf(std::powf(delta.p, 2.0f) + std::powf(delta.y, 2.0f));
    }
}
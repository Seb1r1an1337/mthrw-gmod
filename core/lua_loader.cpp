...
void lualoader::Init() noexcept
{
	...
	luaL_loadstring = reinterpret_cast<_luaL_loadstring>(GetProcAddress(h_luashared, "luaL_loadstring"));
	        ...
	        lua_getfield = reinterpret_cast<_lua_getfield>(GetProcAddress(h_luashared, "lua_getfield"));
	        lua_pushnumber = reinterpret_cast<_lua_pushnumber>(GetProcAddress(h_luashared, "lua_pushnumber"));
	        lua_type = reinterpret_cast<_lua_type>(GetProcAddress(h_luashared, "lua_type"));
	}
auto getallthreads() {
...
bool lualoader::Execute(std::string script) noexcept
{
	if (interfaces::engine->IsInGame()) {
		...
		luaL_loadbuffer(config::luastate, script.c_str(), script.size(), "mth_lua_loader");
		if (lua_pcall(config::luastate, 0, 0, 0) != 0) {
